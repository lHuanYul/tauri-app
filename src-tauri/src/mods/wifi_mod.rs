...
pub async fn tcp_send_packet() -> Result<(), Box<dyn error::Error>> {
    let addr = "192.168.0.189:60000";
    let data = b"Hello from Rust by TCP";
    // 1. 建立 TCP 連線（三次握手）
    let mut stream = TcpStream::connect(addr).await.map_err(|e| {
        let message = format!("TCP connect({}) failed: {}", addr, e);
        error!("{}", message);
        message
    })?;
    ...
    stream.write_all(data).await.map_err(|e| {
        let message = format!("TCP send to {} failed: {}", addr, e);
        error!("{}", message);
        message
    })?;
    ...
    stream.shutdown().await.map_err(|e| {
        let message = format!("TCP shutdown failed: {}", e);
        error!("{}", message);
        message
    })?;
    Ok(())
...
impl WifiReceive {
    ...
    pub async fn udp_stop(&mut self) -> Result<(), Box<dyn error::Error>> {
        ...
        match self.udp_handle.take() {
            ...
            Some(handle) => {
                let _ = handle.await;
                info!("UDP task has been stopped");
                Ok(())
            }
        }
    }
    ...
    pub async fn tcp_receive_stop(&mut self) -> Result<(), Box<dyn error::Error>> {
        ...
        match self.tcp_handle.take() {
            ...
            Some(handle) => {
                let _ = handle.await;
                info!("TCP task has been stopped");
                Ok(())
            }
        }
    }
